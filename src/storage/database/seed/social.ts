...
export function seedSocial(db: Drizzle, ctx: SeedCtx) {
  ...
  for (const c of sysConfing) db.insert(schema.systemConfig).values({ id: nextId(), ...c }).run();
  ...
  for (const cid of ctx.courseIds) {
    ...
    for (let m = 0; m < nMat; m++) {
      ...
      db.insert(schema.learningMaterial).values({
      ...
      for (const sid of students) {
        ...
        db.insert(schema.learningBehaviorLog).values({
  ...
  for (const sid of ctx.students) {
    ...
    db.insert(schema.pointsAccount).values({
    ...
    db.insert(schema.signInSummary).values({
    ...
    for (let d = 0; d < days; d++) {
      db.insert(schema.signInRecord).values({
  ...
  for (const sid of ctx.students) {
    ...
    for (let p = 0; p < planCount; p++) {
      ...
      db.insert(schema.studyPlan).values({
      ...
      for (let s = 0; s < sessN; s++) {
        db.insert(schema.studySession).values({
    ...
    if (rng.bool(0.6)) {
      const sessId = nextId();
      db.insert(schema.qaSession).values({
      ...
      db.insert(schema.qaMessage).values({ id: nextId(), session_id: sessId, role: 'user', content: '这道题我不太会，能帮忙讲讲思路吗？' } as any).run();
      db.insert(schema.qaMessage).values({ id: nextId(), session_id: sessId, role: 'assistant', content: '好的，这道题的关键在于抓住核心概念，然后逐步推导。首先...' } as any).run();
    }
  }
  ...
  for (const [cid] of ctx.courseIds.map((cid, i) => [cid, i] as const)) {
    ...
    db.insert(schema.classSchedule).values({
  ...
  for (const sid of ctx.students) {
    ...
    db.insert(schema.studentSchedule).values({
  ...
  for (const cid of ctx.courseIds) {
    ...
    for (let p = 0; p < nPost; p++) {
      ...
      db.insert(schema.discussionPost).values({
      ...
      for (let r = 0; r < nReply; r++) {
        ...
        db.insert(schema.discussionReply).values({
        ...
        if (rng.bool(0.3)) {
          db.insert(schema.discussionLike).values({
            id: nextId(), target_type: 'reply', target_id: rid, user_id: students[rng.int(0, students.length)],
          } as any).run();
        }
      }
      if (rng.bool(0.5)) {
        db.insert(schema.discussionLike).values({ id: nextId(), target_type: 'post', target_id: postId, user_id: students[rng.int(0, students.length)] } as any).run();
      }
    }
  }
  ...
  for (const cid of ctx.courseIds) {
    const teacherId = ctx.courseTeacher.get(cid)!;
    db.insert(schema.announcement).values({
  ...
  for (const [cid, cIdx] of ctx.courseIds.map((cid, i) => [cid, i] as const)) {
    const teacherId = ctx.courseTeacher.get(cid)!;
    db.insert(schema.gradingConfig).values({
    ...
    // 能力点 + 思政点
    const abId = nextId();
    db.insert(schema.abilityPoint).values({ id: abId, name: '分析与建模能力', description: '把实际问题抽象为可求解的模型', course_id: cid } as any).run();
    const kbId = nextId();
    db.insert(schema.ideologyPoint).values({ id: kbId, name: '工程伦理与责任', description: '在工程实践中坚守伦理与职业操守', course_id: cid } as any).run();
    for (const k of ctx.courseKps.get(cid) ?? []) {
      db.insert(schema.abilityKnowledge).values({ id: nextId(), ability_id: abId, knowledge_id: k.kpId, weight: rng.range(0.5, 1) } as any).run();
      if (rng.bool(0.5)) db.insert(schema.ideologyKnowledge).values({ id: nextId(), ideology_id: kbId, knowledge_id: k.kpId } as any).run();
    }
    void cIdx;
  }
  ...
  for (const it of ITEMS) {
    const iid = nextId();
    itemIds.push(iid);
    db.insert(schema.shopItem).values({
  ...
  for (const sid of ctx.students.slice(0, 20)) {
    ...
    db.insert(schema.redeemOrder).values({
    ...
    db.insert(schema.userDecoration).values({
  ...
  for (const [cid, cIdx] of ctx.courseIds.map((cid, i) => [cid, i] as const)) {
    ...
    for (let q = 0; q < rng.int(1, 3); q++) {
      const teacherId = ctx.courseTeacher.get(cid)!;
      db.insert(schema.questionRecord).values({
