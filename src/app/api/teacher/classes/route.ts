...
export async function GET(request: NextRequest) {
  ...
  const myCourses = db.select({ id: course.id, name: course.name, class_id: course.class_id }).from(course).where(eq(course.teacher_id, r.user.userId)).all();
  const classIds = [...new Set(myCourses.map((c) => c.class_id).filter((v): v is number => v != null))];

  let classes: Array<{ id: number; name: string }> = [];
  if (classIds.length) {
    classes = db.select({ id: classInfo.id, name: classInfo.name }).from(classInfo).where(inArray(classInfo.id, classIds)).all();
  }
