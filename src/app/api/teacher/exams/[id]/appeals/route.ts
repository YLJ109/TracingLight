...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const questions = qids.length
    ? new Map(db.select({ id: question.id, content: question.content }).from(question).where(inArray(question.id, qids)).all().map((q) => [q.id, q]))
    : new Map();
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const appeal = db.select().from(examAppeal).where(eq(examAppeal.id, body.appeal_id)).get();
  ...
  if (status === 'resolved' && body.override_score != null && appeal.grading_id) {
    const g = db.select().from(examGrading).where(eq(examGrading.id, appeal.grading_id)).get();
    if (g) {
      ...
      }).where(eq(examGrading.id, appeal.grading_id)).run();

      const isCorrect = full > 0 && newScore >= full;
      // 掌握度回写（与批改/作业同一口径：指数平滑，对题不计错，错题计错）
      syncMasteryFromGrading({ studentId: appeal.student_id, knowledgePointId: (g.knowledge_point_id as number | null) ?? null, score: newScore, fullScore: full, isCorrect });

      // 错题本联动：判对 → 标记已掌握并移除复习排期；判错 → 重置 pending 排期
      // 仅 reconcile 已存在的错题行（该题原始判错时已由提交落库进入错题本），不做部分字段的新增插入
      const eb = db.select().from(errorBook)
        .where(and(eq(errorBook.student_id, appeal.student_id), eq(errorBook.question_id, g.question_id))).get();
      if (eb) {
        const next = !isCorrect
          ? new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().slice(0, 19).replace('T', ' ')
          : null;
        db.update(errorBook).set({ review_status: isCorrect ? 'mastered' : 'pending', next_review_at: next }).where(eq(errorBook.id, eb.id)).run();
      }
    }
  }

  db.update(examAppeal).set({ status, teacher_comment: body.teacher_comment || appeal.teacher_comment, handled_at: now }).where(eq(examAppeal.id, appeal.id)).run();
