...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!getTeacherCourseIds(user.userId).includes(courseId)) {
      return NextResponse.json({ error: '无权查看该课程材料' }, { status: 403 });
    }

    const data = db.select().from(learningMaterial)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const found = db.select({ id: learningMaterial.id, course_id: learningMaterial.course_id })
      .from(learningMaterial)
      .where(inArray(learningMaterial.id, ids))
      .all();
    const scopeCourses = getTeacherCourseIds(user.userId);
    ...
    for (const item of items) {
      ...
      if (Object.keys(patch).length > 0) {
        db.update(learningMaterial).set(patch).where(eq(learningMaterial.id, id)).run();
        updated.push(id);
      }
    }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const target = db.select({ id: learningMaterial.id, course_id: learningMaterial.course_id })
      .from(learningMaterial)
      .where(eq(learningMaterial.id, id))
      .limit(1)
      .all();
    if (!target[0] || !getTeacherCourseIds(user.userId).includes(target[0].course_id)) {
      return NextResponse.json({ error: '无权删除该材料' }, { status: 403 });
    }

    // 先清掉关联的学习行为日志，再删材料，避免因外键约束而失败
    db.delete(learningBehaviorLog).where(eq(learningBehaviorLog.material_id, id)).run();
    db.delete(learningMaterial).where(eq(learningMaterial.id, id)).run();
