...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const enroll = (await db.select().from(examEnroll).where(and(eq(examEnroll.exam_id, examId), eq(examEnroll.student_id, r.user.userId))).execute())[0];
  ...
  let attempt = (await db.select().from(examAttempt).where(and(eq(examAttempt.exam_id, examId), eq(examAttempt.student_id, r.user.userId))).execute())[0];
  ...
  if (!attempt) {
    ...
    attempt = (await db.insert(examAttempt).values({
    ...
    attempt = (await db.select().from(examAttempt).where(eq(examAttempt.id, attempt.id)).execute())[0];
  }
