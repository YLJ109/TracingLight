...
async function countAdmins(db: ReturnType<typeof getDb>): Promise<number> {
  const rows = await db.select({ id: user.id }).from(user).where(eq(user.role, 'admin')).execute();
  return rows.length;
}

// 使该用户的旧 token 全部失效（物理表经迁移已含 token_version 列）
async function bumpTokenVersion(userId: number): Promise<void> {
  await getDb().update(user)
    .set({ token_version: sql`COALESCE(${user.token_version}, 0) + 1` })
...
export async function POST(request: NextRequest) {
  try {
    ...
    const writeAudit = async (action: string, targetId: string, detail: string) => {
    ...
    if (action === 'create') {
      ...
      const exists = await db.select().from(user).where(eq(user.username, username)).execute();
      ...
      await writeAudit('create_user', String(inserted.id), `新增账号 ${username}（${role}）`);
    ...
    const targetRow = (await db.select().from(user).where(eq(user.id, userId)).limit(1).execute())[0];
    ...
    if (action === 'disable' || action === 'enable') {
      ...
      await db.update(user).set({ is_active: action === 'enable' }).where(eq(user.id, userId)).execute();
      // 禁用使旧 token 失效
      if (action === 'disable') {
        await bumpTokenVersion(userId);
      }
      await writeAudit(action === 'disable' ? 'disable_user' : 'enable_user', String(userId), `${action === 'disable' ? '禁用' : '启用'}用户 ${userId}`);
    } else if (action === 'reset_password') {
      const pwd = body.password || '123456';
      if (pwd.length < 6) {
        return NextResponse.json({ error: '密码长度不能少于 6 位' }, { status: 400 });
      }
      await db.update(user).set({ password: hashPassword(pwd) }).where(eq(user.id, userId)).execute();
      // 改密使旧 token 全部失效
      await bumpTokenVersion(userId);
      await writeAudit('reset_password', String(userId), `重置用户 ${userId} 密码`);
      ...
      await bumpTokenVersion(userId);
      await writeAudit('change_role', String(userId), `将用户 ${userId} 角色改为 ${role}`);
    } else {
