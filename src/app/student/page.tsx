...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    apiFetch(`/api/student/profile?student_id=${user.id}`)
      .then(r => r.json())
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid gap-3 grid-cols-2 md:grid-cols-4 animate-stagger">
        {indicators.map((ind, i) => {
          ...
          return (
            <Card key={i} className={`relative overflow-hidden border-slate-200/60 shadow-sm card-hover bg-gradient-to-br ${INDICATOR_GRADIENT[ind.color] || 'from-slate-50 to-white'}`}>
